...
export function FeaturedProducts({ products }: FeaturedProductsProps) {
  ...
  // Duplicate the list so the marquee can loop seamlessly — the track
  // translates -50% and the second copy fills the gap with no visible jump.
    ...
    <>
      <section className="relative overflow-hidden">
        ...
        <div className="section-shell py-16 md:py-24">
          ...
          {/* Infinite horizontal marquee — seamless loop, pauses on hover */}
          <div className="featured-marquee group relative w-full overflow-hidden">
            <div className="featured-marquee__track flex w-max">
              {marqueeItems.map((product, i) => (
                <div
                  key={`${product.id}-${i}`}
                  className="featured-marquee__item w-[260px] shrink-0 px-3 sm:w-[300px]"
                >
                  <div className="card-gold-hover group/card relative overflow-hidden rounded-2xl border border-[rgba(var(--ink-rgb),0.06)] bg-white/80 hover:-translate-y-2">
                    <Link href={`/products/${product.slug}`} className="block">
                      <div className="relative aspect-[3/4] overflow-hidden">
                        ...
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-[rgba(var(--scrim-rgb),0.75)] via-[rgba(var(--scrim-rgb),0.20)] to-transparent opacity-0 transition-opacity duration-500 group-hover/card:opacity-100" />
                        {product.badge && (
                    ...
                    <div className="absolute bottom-0 left-0 right-0 translate-y-4 p-4 opacity-0 transition-all duration-500 group-hover/card:translate-y-0 group-hover/card:opacity-100 z-10">
                      <button
                        type="button"
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-3 flex items-center gap-1 text-xs text-muted">
                        ...
                        {product.rating}
                        <span className="text-[color:var(--text-light)]">({product.reviews})</span>
                      </div>
                    </div>
                  </div>
                </div>
